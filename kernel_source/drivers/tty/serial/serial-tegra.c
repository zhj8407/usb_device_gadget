...
struct tegra_uart_port {
	...
	tegra_wake_peer_t		wake_peer[TEGRA_UART_MAXIMUM];
};

tegra_wake_peer_t tegra_serial_wake_peer[TEGRA_UART_MAXIMUM];
...
static void tegra_uart_wake_peer(struct uart_port *port)
{
	struct tegra_uart_port *tup = to_tegra_uport(port);

	if (tup->wake_peer[port->line])
		tup->wake_peer[port->line](port);
}
...
static int tegra_uart_startup(struct uart_port *u)
{
	...
	tup->wake_peer[u->line] = tegra_serial_wake_peer[u->line];
...
static struct uart_ops tegra_uart_ops = {
	...
	.wake_peer	= tegra_uart_wake_peer,
};
