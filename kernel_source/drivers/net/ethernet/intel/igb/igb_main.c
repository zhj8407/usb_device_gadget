...
static void igb_tsync_interrupt(struct igb_adapter *adapter)
{
		...
		if(tsicr & E1000_TSICR_SYSWARP) { /* (1<<0) */
		...
		if(tsicr & E1000_TSICR_AUTT0) {   /* (1<<5) */
			/* acknowledge the interrupt */
		...
		if(tsicr & E1000_TSICR_AUTT1) { /* (1<<6) */
			/* acknowledge the interrupt */
...
static int igb_mii_ioctl(struct net_device *netdev, struct ifreq *ifr, int cmd)
{
	...
	case SIOCGMIIREG:
		if (!capable(CAP_NET_ADMIN))
			return -EPERM;
		if (e1000_read_phy_reg(&adapter->hw, data->reg_num, &data->val_out))
			return -EIO;
		break;
	case SIOCSMIIREG:
		mii_reg = data->val_in;
