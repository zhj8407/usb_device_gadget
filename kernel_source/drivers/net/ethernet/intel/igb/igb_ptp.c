		...
		struct ptp_clock_request *rq, int on)
{
	...
	switch(rq->type) {
		...
		case PTP_CLK_REQ_PPS:
			...
			if(on) {
				E1000_WRITE_REG(hw, E1000_FREQOUT1, (NSEC_PER_SEC / 2));
				E1000_WRITE_FLUSH(hw);

				/* Map SDP1 to FREQOUT0 */
				regval = E1000_READ_REG(hw, E1000_TSSDP);
				regval |= E1000_TS_SDP1_SEL(3) | E1000_TS_SDP1_EN;
				E1000_WRITE_REG(hw, E1000_TSSDP, regval);
				/* Set SDP1 to output */
				regval = E1000_READ_REG(hw, E1000_CTRL);
				regval |= ( E1000_TS_SDP1_DIR );
				...
				E1000_WRITE_REG(hw, E1000_TRGTTIMH1, ts.tv_sec);
				E1000_WRITE_FLUSH(hw);
				...
				/*Enable FREQOUT0 */
				regval = E1000_READ_REG(hw, E1000_TSAUXC);
				regval |= (E1000_TSAUXC_EN_CLK1 | E1000_TSAUXC_ST1 | E1000_TSAUXC_EN_TT1);
				E1000_WRITE_REG(hw, E1000_TSAUXC, regval); 
				E1000_WRITE_FLUSH(hw);
			}
			else {
				regval = E1000_READ_REG(hw, E1000_TSAUXC);
				regval &= ~E1000_TSAUXC_EN_CLK1;
				E1000_WRITE_REG(hw, E1000_TSAUXC, regval);
