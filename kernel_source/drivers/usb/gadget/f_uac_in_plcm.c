...
static int snd_uac_plcm_probe(struct platform_device *pdev)
{
	 ...
	 */
	err = snd_pcm_new(uac_plcmc->card, "Plcm PCM Source", 0, 1, 0, &pcm);
	if (err < 0)
		goto snd_fail;

	strcpy(pcm->name, "Plcm PCM Source");
...
static struct uac_input_terminal_descriptor mic_input_terminal_desc = {
	...
	.wTerminalType = 	__constant_cpu_to_le16(UAC_INPUT_TERMINAL_MICROPHONE),
	.bAssocTerminal = 	0,
...
static struct usb_interface_descriptor mic_as_intf_alt0_desc = {
	...
	.bDescriptorType = 	USB_DT_INTERFACE,
	.bInterfaceNumber = 	0x00,	/* Dynamically. */
	.bAlternateSetting = 	0x00,
	.bNumEndpoints = 	0x00,
	.bInterfaceClass = 	USB_CLASS_AUDIO,
...
static void audio_status_notify(struct work_struct *data)
{
	...
	if (uevent_envp) {
		...
		pr_info("%s: audio did not send uevent (suspended: %d, source_active: %d\n",
			__func__, agdev->suspended, agdev->source_active);
	}
}
