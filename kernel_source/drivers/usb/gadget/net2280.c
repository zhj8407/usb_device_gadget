...
static void ep_reset_228x(struct net2280_regs __iomem *regs, struct net2280_ep *ep)
{
	u32		tmp;
...
net2280_fifo_flush (struct usb_ep *_ep)
{
	...
	if (!_ep || (!ep->desc && ep->num != 0))
		return;
	if (!ep->dev->driver || ep->dev->gadget.speed == USB_SPEED_UNKNOWN){
		printk(KERN_ERR "%s %d fail unknown\n", __func__,__LINE__);		
...
static int net2280_pullup(struct usb_gadget *_gadget, int is_on)
{
	...
	if (!_gadget)
		{
			printk(KERN_ERR "[net2280]%s %d fail\n", __func__,__LINE__); 			
			return -ENODEV;
		}
	dev = container_of (_gadget, struct net2280, gadget);
...
static void defect7374_enable_data_eps_zero(struct net2280 *dev)
{
	...
	if (fsmvalue != DEFECT7374_FSM_SS_CONTROL_READ) {
		...
		/*Implemented for development and debug. * Can be refined/tuned later.*/
		for (ep_sel = 0; ep_sel <= 21; ep_sel++) {
			/* Select an endpoint for subsequent operations: */
			tmp_reg = readl(&dev->plregs->pl_ep_ctrl);
...
{
	printk(KERN_ERR "[net2280]%s %d dev->pdev->vendor=%d\n", __func__,__LINE__,dev->pdev->vendor); 			
	if (dev->pdev->vendor == 0x17cc)
...
{
	printk(KERN_ERR "%s %d dev->pdev->vendor=%d\n", __func__,__LINE__,dev->pdev->vendor); 			
	if (dev->pdev->vendor == 0x17cc)
...
get_ep_by_addr (struct net2280 *dev, u16 wIndex)
{
	...
	printk("[net2280]%s %d %s return NULL\n",__func__,__LINE__);
	return NULL;
}
	...
	if (unlikely (stat & (1 << SETUP_PACKET_INTERRUPT))) {
		...
		if (dev->gadget.speed == USB_SPEED_UNKNOWN) {
			...
			if (val & (1 << SUPER_SPEED)) {
				...
				usb_ep_set_maxpacket_limit(&dev->ep[0].ep, EP0_SS_MAX_PACKET_SIZE);
			} else if (val & (1 << HIGH_SPEED)) {
				dev->gadget.speed = USB_SPEED_HIGH;
				usb_ep_set_maxpacket_limit(&dev->ep[0].ep, EP0_HS_MAX_PACKET_SIZE);
			} else {
		...
		if (dev->pdev->vendor == 0x10b5)
			...
			if (ep->dev->pdev->device == 0x2280)
				tmp = (1 << FIFO_OVERFLOW) | (1 << FIFO_UNDERFLOW);
			else
				tmp = 0;
...
#define	w_length	le16_to_cpu(u.r.wLength)
		...
		case USB_REQ_CLEAR_FEATURE: {
			...
			if (e->wedged) {
				...
				VDEBUG(dev, "%s clear halt\n", e->ep.name);
				clear_halt(e);
				if (ep->dev->pdev->vendor == 0x10b5 && !list_empty(&e->queue) && e->td_dma)
					restart_dma(e);				
			}
			allow_status (ep);
			goto next_endpoints;
		...
		case USB_REQ_SET_FEATURE: {
			...
			printk("[net2280]%s %d %s set halt\n",__func__,__LINE__,ep->ep.name);
...
next_endpoints:
	...
	if (stat){
		printk("[net2280]%s %d unhandled irqstat0 %08x\n",__func__,__LINE__,stat);
...
static void handle_stat1_irqs (struct net2280 *dev, u32 stat)
{
	...
	if (stat & tmp) {
		...
		if ((((stat & (1 << ROOT_PORT_RESET_INTERRUPT))
			...
			printk("[net2280]%s %d disconnect %s\n",__func__,__LINE__,dev->driver->driver.name);
	...
	for (num = 0; scratch; num++) {
		...
		if (dev->pdev->vendor == 0x10b5) {
			...
			if (!ep->is_in &&  (r_dmacount & 0x00FFFFFF) &&	(tmp & (1 << DMA_TRANSACTION_DONE_INTERRUPT)))
				continue;
		}
		...
		if (!use_dma_chaining) {
			if (!(tmp & (1 << DMA_TRANSACTION_DONE_INTERRUPT))) {
				DEBUG (ep->dev, "%s no xact done? %08x\n",
					ep->ep.name, tmp);
				printk("[net2280]%s %d s no xact done? %08x\n",__func__,__LINE__,ep->ep.name, tmp);
	...
	if (stat & PCI_ERROR_INTERRUPTS) {
		...
		printk("[net2280]%s %d pci dma error; stat %08x\n",__func__,__LINE__,stat);
	...
	if (stat){
		printk("[net2280]%s %d unhandled irqstat1 %08x\n",__func__,__LINE__,stat);
...
static irqreturn_t net2280_irq (int irq, void *_dev)
{
	...
	if (dev->pdev->vendor == 0x17cc && (!(readl(&dev->regs->irqstat0) & (1 << INTA_ASSERTED)))){
		printk("[net2280]%s %d IRQ_NONE\n",__func__,__LINE__);
...
printk("[net2280]%s %d\n",__func__,__LINE__);
	...
	dev->gadget.max_speed = (dev->pdev->vendor == 0x10b5) ?	USB_SPEED_SUPER : USB_SPEED_HIGH;
	...
	if (dev->pdev->vendor == 0x10b5) {
		u32 fsmvalue;
		u32 usbstat;
		dev->usb_ext = (struct usb338x_usb_ext_regs __iomem *)(base + 0x00b4);
		...
		fsmvalue = get_idx_reg(dev->regs, SCRATCH) & (0xf << DEFECT7374_FSM_FIELD);
		/* See if firmware needs to set up for workaround: */
		if (fsmvalue == DEFECT7374_FSM_SS_CONTROL_READ)
	...
	INFO(dev, "version: " DRIVER_VERSION "; dma %s %s\n",use_dma ? (use_dma_chaining ? "chaining" : "enabled"): "disabled",dev->enhanced_mode ? "enhanced mode" : "legacy mode");
