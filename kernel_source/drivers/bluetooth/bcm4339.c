...
static int bcm_bt_lpm_init(struct platform_device *pdev)
{
	...
	tegra_serial_wake_peer[BT_UPORT] = (tegra_wake_peer_t) bcm_bt_lpm_exit_lpm_locked;
...
static int bcm4339_bluetooth_probe(struct platform_device *pdev)
{
	...
	res = platform_get_resource_byname(pdev, IORESOURCE_IO, "bten_gpio");
	if (res) {
		bt_gpio.bt_en = res->start;
		...
		//pr_err("[BT] bt_gpio.bt_en not registered\n");
		return rc;
	}
	
	res = platform_get_resource_byname(pdev, IORESOURCE_IO, "btwake_gpio");
	if (res) {
		bt_gpio.bt_wake = res->start;
		rc = gpio_request(bt_gpio.bt_wake, "bt_wake");
		if (unlikely(rc)) {
			//pr_err("[BT] bt_gpio.bt_wake request failed.\n");
			return rc;
		}
	} else {
		//pr_err("[BT] bt_gpio.bt_wake not registered\n");
		return rc;
	}
	
	res = platform_get_resource_byname(pdev, IORESOURCE_IO, "bthostwake_gpio");
	if (res) {
		bt_gpio.bt_hostwake = res->start;
		rc = gpio_request(bt_gpio.bt_hostwake, "bt_hostwake");
		if (unlikely(rc)) {
			//pr_err("[BT] bt_gpio.bt_hostwake request failed.\n");
			return rc;
		}
	} else {
		//pr_err("[BT] bt_gpio.bt_hostwake not registered\n");
		return rc;
	}
