...
static void v4l_print_vout_pipeline(const void *arg, bool write_only) {
	return;
}

static int v4l_vout_pipeline(const struct v4l2_ioctl_ops *ops, struct file *file, void *fh, void *arg) {
	
	struct v4l2_vout_pipeline *p = (struct v4l2_vout_pipeline *) arg;	
	
	if (ops->vidioc_vout_pipeline)
		return ops->vidioc_vout_pipeline(file, fh, p);
	
	return -EINVAL;
}

static void v4l_print_scaler_crop(const void *arg, bool write_only) {
	return;
}

static int v4l_scaler_crop(const struct v4l2_ioctl_ops *ops, struct file *file, void *fh, void *arg) {
	
	struct v4l2_scaler_crop *p = (struct v4l2_scaler_crop *) arg;
	
	if (ops->vidioc_scaler_crop)
		return ops->vidioc_scaler_crop(file, fh, p);
...
static void v4l_print_uart_mode(const void *arg, bool write_only) {
	return;
}

static int v4l_uart_mode(const struct v4l2_ioctl_ops *ops, struct file *file, void *fh, void *arg) {
	
	struct  v4l2_uart_mode *p = (struct v4l2_uart_mode *) arg;	
	
	if (ops->vidioc_uart_mode)
		return ops->vidioc_uart_mode(file, fh, p);
...
static struct v4l2_ioctl_info v4l2_ioctls[] = {
	...
	IOCTL_INFO_FNC(VIDIOC_S_UART_MODE, v4l_uart_mode, v4l_print_uart_mode, 0)
