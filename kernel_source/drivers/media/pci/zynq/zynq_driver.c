				...
				const struct pci_device_id *ent)
{
	...
	zynq_printk(0, "[zynq_driver]%s: found at %s, bus: %d, rev: %d, irq: %d, "
...
{
	printk(KERN_INFO "[zynq_driver]zynq driver version %s loaded\n",
		ZYNQ_VERSION);
...
{
	printk(KERN_INFO "[zynq_driver]zynq driver version %s unloaded\n",
		ZYNQ_VERSION);
