...
#else
	zynq_printk(0,"[zynq_uart]Enter zynq_uart_start_tx()\n");
	tiny_tx_char(port);	
	 zynq_printk(0,"[zynq_uart]Leave zynq_uart_start_tx()\n");
#endif	
...
int zynq_uart_probe( struct pci_dev *pdev)
{
	...
	if (init_uart_ports(pdev) !=  0) 
	{
		...
		zynq_printk(0,"[zynq_uart]Failed to call init_uart_ports()!!\n");
		return  -1;
	}
...
int Dequeue(int port_id, char *ch) {
	...
	if(isEmpty(port_id)) return -1;  // Return null character if queue is empty
	else
	{
		heads[port_id]++;
...
static void tiny_tx_char(struct uart_port *port)
{
	...
	struct zynq_uart_t  *up = port_to_zynq_uart(port);
	struct circ_buf *xmit  = &port->state->xmit;
	zynq_printk(0,"[zynq_uart]Enter  tiny_tx_char()\n");
	if (port->x_char) {
		port->x_char = 0;
		port->icount.tx++;
		return;
	...
	zynq_printk(0,"[zynq_uart]Leave  tiny_tx_char()\n");
...
static void tiny_rx_char(struct uart_port *port)
{
	...
	int ret = -1;
	char flag = TTY_NORMAL;
	zynq_printk(0,"[zynq_uart]Enter  tiny_rx_char()\n");
...
exit:	
	zynq_printk(0,"[zynq_uart]Leave  tiny_rx_char()\n");
}
			     ...
			     struct ktermios *new, struct ktermios *old)
{
	...
	zynq_printk(0,"[zynq_uart]Enter  tiny_set_termios()\n");
	/* get the byte size */
	switch (cflag & CSIZE) {
	case CS5:
	...
	zynq_printk(0,"[zynq_uart]Leave  tiny_set_termios()\n");
...
static int tiny_start_rx(struct uart_port *port)
{
	...
	zynq_printk(0,"[zynq_uart]Enter  tiny_start_rx()\n");
#if 1
	...
	add_timer(&timer);
#endif
	zynq_printk(0,"[zynq_uart]Leave  tiny_start_rx()\n");
	return 0;
}
static void tiny_stop_rx(struct uart_port *port)
{
	...
	zynq_printk(0,"[zynq_uart]Enter  tiny_stop_rx()\n");
#if 1	
	del_timer(&timer);
	ClearQueue(up->idx);
#endif
	zynq_printk(0,"[zynq_uart]Leave  tiny_stop_rx()\n");
}
#endif
