...
static int snd_i2s_trigger(struct snd_pcm_substream *substream, int cmd)
{
    ...
    switch (cmd) {
        ...
        case SNDRV_PCM_TRIGGER_SUSPEND:
            zynq_printk(1, "[zynq_audio] The SNDRV_PCM_TRIGGER_SUSPEND has been called!!\n");
#ifdef VIRTUAL_AUDIO_DEVICE
            minivosc_timer_stop(chip->virtual_dev, substream->stream);
#endif
            break;
        case SNDRV_PCM_TRIGGER_RESUME:
            zynq_printk(1, "[zynq_audio] The SNDRV_PCM_TRIGGER_RESUME  has been called!!\n");
#ifdef VIRTUAL_AUDIO_DEVICE
            minivosc_timer_start(chip->virtual_dev, jiffies, substream->stream);
#endif
            break;
...
#endif
    if (err < 0) {
        zynq_printk(0,"[zynq_audio]Pre-allocate memory fail.\n");
        return err;
    }
    switch (dev) {
        case MOZART_I2S0:
...
int zynq_audio_init (struct pci_dev *pdev)
{
    ...
    //zynq_printk(1,"[zynq_audio] Zynq Audio Switch Register Base Address : 0x%p\n", zynq_audio_reg_base);

    ret =snd_i2s_probe(pdev);

    if (ret == 0) {
        zynq_printk(1,  "[zynq_audio]The audio function initialized !!\n");
    } else {
        zynq_printk(0,  "[zynq_audio]Failed to initialize  audio function !!\n");
    }
    return ret;
...
int zynq_audio_release(struct pci_dev *pdev)
{
    ...
    if (ret == 0) {
        zynq_printk(1,  "[zynq_audio]The audio function released !!\n");
    } else {
        zynq_printk(0,  "[zynq_audio]Failed to release  audio function !!\n");
    }
    return ret;
}
