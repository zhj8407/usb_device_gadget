...
#include "adv7511_priv.h"

static int debug;
module_param(debug, int, 0644);
MODULE_PARM_DESC(debug, "debug level (0-2)");

static int display_mode = 0; //0:1080p@60, 1:720p@60
module_param(display_mode, int, 0644);
MODULE_PARM_DESC(display_mode, "display mode (0 or 1)");
...
#define ADV7511_DRIVER_NAME "adv7511"
...
#define ADV7511_DEFAULT_INPUT_VIDEO_CLK (148500) //148500 KHz
...
//refere from : http://lxr.free-electrons.com/source/include/uapi/linux/v4l2-dv-timings.h
dv_timings_t  dv_times_1080p60 = {
		.value = V4L2_DV_BT_CEA_1920X1080P60
}; 

dv_timings_t  dv_times_720p60 = {
		.value = V4L2_DV_BT_CEA_1280X720P60
}; 

//////////////////////////////////////////////////////////////////////////////////////////
static void adv7511_set_proper_operation_regs(struct adv7511_state *state);
static void adv7511_set_default_config (struct adv7511_state *state) ;
static void adv7511_set_default_mode (struct adv7511_state *state, unsigned int mode);
//////////////////////////////////////////////////////////////////////////////////////////

static inline void adv7511_delay(unsigned int sec) {
...
static void adv7511_csc_rgb_full2limit(struct v4l2_subdev *sd, bool enable)
{
	if (enable) {
		...
		adv7511_wr(sd, 0x1b, 0xad);
		adv7511_wr(sd, 0x1c, 0x00);
		adv7511_wr(sd, 0x1d, 0x00);
		adv7511_wr(sd, 0x1e, 0x1c);
		adv7511_wr(sd, 0x1f, 0x1b);
		printk(KERN_INFO">>>>>>>>>>>>>>>>>>>>%d\n", __LINE__);
		adv7511_wr(sd, 0x20, 0x1d);
		printk(KERN_INFO">>>>>>>>>>>>>>>>>>>>%d\n", __LINE__);
		adv7511_wr(sd, 0x21, 0xdc);
		adv7511_wr(sd,  0x22, 0x04);
		adv7511_wr(sd,  0x23, 0xad);
		adv7511_wr(sd,  0x24, 0x1f);
		adv7511_wr(sd,  0x25, 0x24);
		adv7511_wr(sd,  0x26, 0x01);
		adv7511_wr(sd,  0x27, 0x35);
		adv7511_wr(sd,  0x28, 0x00);
		adv7511_wr(sd,  0x29, 0x00);
		adv7511_wr(sd,  0x2a, 0x04);
		adv7511_wr(sd,  0x2b, 0xad);
		adv7511_wr(sd,  0x2c, 0x08);
...
static int adv7511_isr(struct v4l2_subdev *sd, u32 status, bool *handled)
{
	...
	v4l2_dbg(0, debug, sd, "[adv7511] %s: irq 0x%x\n", __func__, irq_status);
...
static int adv7511_enum_mbus_code(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh, struct v4l2_subdev_mbus_code_enum *code) {
...
static void adv7511_fill_format(struct adv7511_state *state,
	...
	format->quantization = V4L2_DV_RGB_RANGE_FULL;
}

static int adv7511_get_fmt(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh, struct v4l2_subdev_format *format) {
...
static int adv7511_set_fmt(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh, struct v4l2_subdev_format *format) {
	...
	switch (format->format.code) {
	case MEDIA_BUS_FMT_UYVY8_1X16:
		adv7511_wr_and_or(sd, 0x16, 0xef, 0x80);
	...
	case MEDIA_BUS_FMT_RGB888_1X24:
	default:
		adv7511_wr_and_or(sd, 0x16, 0xef, 0x00);
		break;
	}
	...
	if (format->format.code == MEDIA_BUS_FMT_RGB888_1X24 ) return  0;
...
static void adv7511_setup(struct v4l2_subdev *sd)
{
	...
	adv7511_wr_and_or(sd, 0x55, 0x9c, 0x12);
	 ...
	 adv7511_set_default_config(state);
	
	 adv7511_set_default_mode (state, display_mode);
	 
	/* AVI Info frame packet enable, Audio Info frame disable */
	adv7511_wr_and_or(sd, 0x44, 0xe7, 0x10);
	
	/* no Colorimetry, Active format aspect ratio: same as picure. */
...
static irqreturn_t adv7511x_irq_handler(int irq, void *devid) {
	
	bool subdev_handled = false;
	u32 status = 0;
	int handled = 0;
	struct adv7511_state *state = (struct adv7511_state *)devid;
	
	struct v4l2_subdev *sd = (struct v4l2_subdev *)&(state->sd);
	
	v4l2_subdev_call(sd, core,  interrupt_service_routine, status, &subdev_handled);
...
static void adv7511_core_init(struct v4l2_subdev *sd,  unsigned int i2c_dev_addr ) {
	//adv7511_wr(struct v4l2_subdev *sd, u8 reg, u8 val)

	unsigned int hpd_gpio_pin = 0;
	int ret = 0;
	if (i2c_dev_addr == 0x39) {
		 hpd_gpio_pin = 68;
	} else if (i2c_dev_addr == 0x3d) {
		hpd_gpio_pin = 23;
	} else {
		return;
	}
	
	ret = gpio_request(hpd_gpio_pin, "ADV7511 HPD pin");
	if (ret) {
		v4l2_err(sd, "%s: gpio_request() for %u  error\n", __func__, hpd_gpio_pin);
		return;
	}
	gpio_direction_output(hpd_gpio_pin, 1);
	
	gpio_free(hpd_gpio_pin);
		
	v4l2_info(sd, "Enetr adv7511_core_init().\n");
	...
	adv7511_wr(sd, 0x98, 0x03); // Recommended write_7511
	adv7511_wr(sd, 0x9a, 0xe0); // Recommended write_7511
	adv7511_wr(sd, 0x9c,  0x30); // Recommended write_7511
	adv7511_wr(sd, 0x9d,  0x61); // Recommended write_7511
	adv7511_wr(sd, 0xa2,  0xa4);// Recommended write_7511
	adv7511_wr(sd, 0xa3, 0xa4);// Recommended write_7511
	adv7511_wr(sd, 0xe0,  0xd0); //Recommended write_7511
	adv7511_wr(sd, 0xf9,  0x00);//Recommended write_7511
...
static int adv7511_remove(struct i2c_client *client)
{
	...
	//	adv7511_init_setup(sd);
	adv7511_wr(sd, 0x41, 0x50);//Power down the txr
	if (state->i2c_edid != NULL) i2c_unregister_device(state->i2c_edid);
	if (state->work_queue != NULL){
...
/*refere: (1)http://referencehometheater.com/2014/commentary/rgb-full-vs-limited/  (2)http://tieba.baidu.com/p/2732517788
 *  (1)TVs use a video range from 16-235. It considers levels below 16 to be black, and information above 235 is white. A calibrated TV will never display anything below 16 as anything other than black. Most will also treat everything over 235 as white since it should not exist in video content.
 *  (2)PCs are different and use a range from 0-255. There is no data below 0 or above 255 with an 8-bit video signal as there are only 256 possible values. In short, this is much simpler to understand as the TV concepts of Blacker-than-Black and Whiter-than-White do not exist.
 */

#if 0
/*HDTV YCbCr (16to 235) to RGB (16to 235)*/
static uint16_t adv7511_csc_ycbcr_to_rgb0[] = {
	0xac53, 0x0800, 0x0000, 0x19d6,
	0x1c56, 0x0800, 0x1e88, 0x0291,
	0x1fff, 0x0800, 0x0e85, 0x18be,
};
#endif
/*HDTV YCbCr (16to 235) to RGB (0 to 255)*/
static uint16_t adv7511_csc_ycbcr_to_rgb1[] = {
...
static struct adv7511_video_input_config adv7511_config_default = {
	...
	.output_format = ADV7511_OUTPUT_FORMAT_RGB_444,
	.csc_enable = true,
	.csc_coefficents = adv7511_csc_ycbcr_to_rgb1,
	.csc_scaling_factor = ADV7511_CSC_SCALING_4,
	.bit_justification = ADV7511_INPUT_BIT_JUSTIFICATION_LEFT,
	.tmds_clock_inversion = false,
	.timing_gen_sequence = 0
};
...
static void adv7511_set_proper_operation_regs(struct adv7511_state *state){
	...
	for (i = 0; i < ARRAY_SIZE(adv7511_fixed_registers); ++i) {
		adv7511_wr(sd, adv7511_fixed_registers[i][0], adv7511_fixed_registers[i][1]);
	}
}
static void adv7511_set_colormap(struct adv7511_state *state, bool enable,
...
#define ADV7511_REG_CSC_LOWER(x)		(0x19 + (x) * 2)
	 ...
	 */
	adv7511_wr_and_or(sd, ADV7511_REG_CSC_UPPER(1), ~(0x20), ADV7511_CSC_UPDATE_MODE);

	if (enable) {
		for (i = 0; i < 12; ++i) {
			adv7511_wr_and_or(sd, ADV7511_REG_CSC_UPPER(i), ~(0x1f), coeff[i] >> 8);
			adv7511_wr(sd, ADV7511_REG_CSC_LOWER(i), coeff[i] & 0x00ff);
		}
	}

	adv7511_wr_and_or(sd,  ADV7511_REG_CSC_UPPER(0), 0x1f, (enable << 7) | (scaling_factor << 5));

	adv7511_wr_and_or(sd,  ADV7511_REG_CSC_UPPER(1), ~(0x20), 0);
}


static void adv7511_set_config(struct adv7511_state *state,  struct adv7511_video_input_config  *c) {
	struct v4l2_subdev *sd = &(state->sd);
	...
	unsigned int mode;
#if 1
	adv7511_set_colormap(state, config->csc_enable,
	...
	case ADV7511_INPUT_ID_8_10_12BIT_YCbCr422_EMBEDDED_SYNC:
		state->embedded_sync = true;
		break;
	default:
		state->embedded_sync = false;
		break;
	}
	
	adv7511_wr_and_or(sd, ADV7511_REG_I2C_FREQ_ID_CFG, ~(0x0f), config->id);
	
	adv7511_wr(sd, ADV7511_REG_VIDEO_INPUT_CFG1,
		(output_format_422 << 7) |
		(config->input_color_depth << 4) |
		(config->input_style << 2) |
		output_format_ycbcr);
	
	adv7511_wr(sd, ADV7511_REG_VIDEO_INPUT_CFG2,
	...
	*/
	adv7511_wr(sd, ADV7511_REG_TIMING_GEN_SEQ,
		(sync_pulse << 2) |
		(config->timing_gen_sequence << 1));
	
	adv7511_wr(sd, 0xba, (config->clock_delay << 5));

	adv7511_wr_and_or(sd,  ADV7511_REG_TMDS_CLOCK_INV, ~(0x08), config->tmds_clock_inversion << 3);

	adv7511_wr_and_or(sd,   ADV7511_REG_AVI_INFOFRAME(0), ~(0x60), config->output_format << 5);
	...
	adv7511_wr_and_or(sd,  ADV7511_REG_HDCP_HDMI_CFG, ~(0x2), mode);
	
	state->vsync_polarity = ADV7511_SYNC_POLARITY_PASSTHROUGH;
	state->hsync_polarity = ADV7511_SYNC_POLARITY_PASSTHROUGH;
}


static void adv7511_set_default_config (struct adv7511_state *state) {
	
	adv7511_set_config(state,  &adv7511_config_default);
}
 ...
 * Validating Mode "1280x720": 1280 x 720 @ 60 Hz
 * Mode Source: EDID
 * Pixel Clock      : 74.25 MHz
 * HRes, HSyncStart :  1280, 1390
 * HSyncEnd, HTotal : 1430, 1650
 * VRes, VSyncStart :  720, 725
 * VSyncEnd, VTotal : 730, 750
 * H/V Polarity     : +/+
 * Mode is valid
 */
#if 0
static struct drm_display_mode display_mode_for_1280x720_60HZ = {
	.clock = 74250, 	/* in kHz  (74.25 MHz)*/
	.hdisplay = 1280,
	.hsync_start = 1390,
	.hsync_end = 1430,
	.htotal = 1650,
 ...
 * HRes, HSyncStart :  1920, 2008
 * HSyncEnd, HTotal : 2052, 2200
 * VRes, VSyncStart : 1080, 1084
 * VSyncEnd, VTotal : 1088, 1152
 * H/V Polarity     : -/-
 * Mode is valid
 */
#if 0
static struct drm_display_mode display_mode_for_1920x1080_60HZ = {
	.clock = 148500, 	/* in kHz  (148.50 MHz)*/
	.hdisplay = 1920,
	.hsync_start = 2008,
	.hsync_end = 2052,
	.htotal = 2200,
...
static struct drm_display_mode display_mode_for_1920x1080_60HZ ={
	DRM_MODE("1920x1080p", DRM_MODE_TYPE_DRIVER, 148500, 1920, 2008, 2052, 2200, 0, 1080, 1084, 1088, 1152, 0, DRM_MODE_FLAG_PHSYNC | DRM_MODE_FLAG_PVSYNC),
	.vrefresh = 60, 
	.hsync = 68,
//	.picture_aspect_ratio = HDMI_PICTURE_ASPECT_16_9
};
static void adv7511_encoder_mode_set(struct adv7511_state *state, struct drm_display_mode *mode) {
	
	struct v4l2_subdev *sd = &(state->sd);
	unsigned int low_refresh_rate = 0;
	unsigned int hsync_polarity = 0;
	unsigned int vsync_polarity = 0;
	
	if (state->embedded_sync) {
		...
		adv7511_wr(sd, ADV7511_REG_HSYNC_PLACEMENT_MSB,
			((hsync_offset >> 10) & 0x7) << 5);
		adv7511_wr(sd,  ADV7511_REG_SYNC_DECODER(0),
			(hsync_offset >> 2) & 0xff);
		adv7511_wr(sd,  ADV7511_REG_SYNC_DECODER(1),
			((hsync_offset & 0x3) << 2) | (hsync_len >> 4));
		adv7511_wr(sd,  ADV7511_REG_SYNC_DECODER(2),
			((hsync_len & 0xf) << 4) | (vsync_offset >> 6));
		adv7511_wr(sd,  ADV7511_REG_SYNC_DECODER(3),
			((vsync_offset & 0x3f) << 2) | (vsync_len >> 8));
		adv7511_wr(sd,  ADV7511_REG_SYNC_DECODER(4),
			vsync_len & 0xff);
		...
		if (state->hsync_polarity != mode_hsync_polarity && state->hsync_polarity != ADV7511_SYNC_POLARITY_PASSTHROUGH)
			hsync_polarity = 1;

		if (state->vsync_polarity != mode_vsync_polarity && state->vsync_polarity != ADV7511_SYNC_POLARITY_PASSTHROUGH)
			vsync_polarity = 1;
	}
	...
	adv7511_wr_and_or(sd,   0xfb, ~(0x06), low_refresh_rate << 1);

	//0x17[1] (Aspect ratio of input video):
	//0 = 4:3 Aspect Ratio
	//1 = 16:9 Aspect Ratio
	adv7511_wr_and_or(sd, 0x17, ~(0x62), (vsync_polarity << 6) | (hsync_polarity << 5)| (1 << 1));
	
	/*
	 0x56[7:6] (Colorimetry (AVI InfoFrame)):
	00 = no data
	01 = ITU601
	10 = ITU709
	11 = Extended Colorimetry Information Valid (Indicated in register 0x57[6:4]
	 
	 0x56[5:4] (Picture Aspect Ratio (AVI InfoFrame)):
	00 = no data
	01 = 4:3
	10 = 16:9
	11 = None
	 
	 0x56[3:0](Active Format Aspect Ratio (AVI InfoFrame))
	...
	adv7511_wr(sd, 0x55, 0x00);
	
	adv7511_wr(sd, 0x40, 0x80);//GC Packet Enabled
	
	if (mode->clock == ADV7511_DEFAULT_INPUT_VIDEO_CLK){
		adv7511_wr_and_or(sd,   0x9d, ~(0x0c), 0x0);
	} else {
		if (mode->clock < ADV7511_DEFAULT_INPUT_VIDEO_CLK) {
			unsigned int divisor = ADV7511_DEFAULT_INPUT_VIDEO_CLK / mode->clock;
			if (divisor == 1) 
				adv7511_wr_and_or(sd,   0x9d, ~(0x0c), 0x00);
			else if (divisor == 2)
				adv7511_wr_and_or(sd,   0x9d, ~(0x0c), 0x01);
			else if (divisor == 4)
				adv7511_wr_and_or(sd,   0x9d, ~(0x0c), 0x02);
		}
	}
	
	state->f_tmds = mode->clock;
}

static void adv7511_set_default_mode (struct adv7511_state *state, unsigned int mode) {
	if (mode == 0)  
		adv7511_encoder_mode_set(state, &display_mode_for_1920x1080_60HZ );
	else if (mode == 1) 
		adv7511_encoder_mode_set(state, &display_mode_for_1280x720_60HZ); 
}
///////////////////////////////////////////////////////////////////////////////////////////////