...
extern struct clk *tegra_ptr_camera_mclks[2];
...
#ifdef CONFIG_VIDEO_TC358746
static struct i2c_board_info __initdata tc358746_board_info = {
	I2C_BOARD_INFO(TC358746_NAME, TC358746_I2C_ADDR),
};
#endif

#ifdef CONFIG_VIDEO_M10MO
static struct i2c_board_info __initdata m10mo_i2c_board_info = {
	I2C_BOARD_INFO(M10MO_NAME, M10MO_I2C_ADDR),
};
#endif
...
#ifdef CONFIG_VIDEO_M10MO
static struct spi_board_info m10mo_spi_board_info[1] = {
	{
		.modalias = M10MO_NAME,
		.bus_num = M10MO_SPI_BUSNUM,
		.chip_select = M10MO_SPI_CS,
		.max_speed_hz = M10MO_SPI_MAX_SPEED_HZ,
...
#ifdef CONFIG_VIDEO_M10MO
    // Enable CPU MCLK to 24MHz
	err = clk_set_rate(tegra_ptr_camera_mclks[0], mclk_init_rate);
	if (!err)
		err = clk_prepare_enable(tegra_ptr_camera_mclks[0]);
	printk(KERN_ALERT"m10mo %s: enable MCLK with %lu Hz, err=%d..\n", __func__, mclk_init_rate,err);

    printk(KERN_ALERT"m10mo %s: register i2c/spi board_info\n", __func__);
	i2c_register_board_info(M10MO_I2C_BUSNUM, &m10mo_i2c_board_info, 1);
    spi_register_board_info(m10mo_spi_board_info, 1);
#endif

#ifdef CONFIG_VIDEO_TC358746
    printk(KERN_ALERT"tc358746 %s: register i2c/spi board_info\n", __func__);
	i2c_register_board_info(TC358746_I2C_BUSNUM, &tc358746_board_info, 1);
#endif
