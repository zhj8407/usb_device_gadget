...
{
    //const unsigned char *tmp = (unsigned char*)&usb_dev->event;
    const unsigned short value = usb_dev->wValue;
    printf("notify_usb_mute_ctl value=0x%x\n", value);

    if (usb_dev->event.bUnitId != MIC_INPUT_FEATURE_UNIT_ID) {
        return DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
    }
...
void set_usb_ctrl_value(struct usb_uac_device *usb_dev, struct uac_ctrl_event *ctr_event)
{
    ...
    usb_dev->wValue = ctr_event->u.fu.value;
}
void listen_to_dbus_event(struct usb_uac_device * usb_dev)
{
    int ret;
...
#endif
            notify_usb_mute_volume_ctl(usb_dev);
        } else {
