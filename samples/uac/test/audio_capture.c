...
// #define DEBUG
//#define TEST_MODE
#define UAC_AUDIO_DEVICE_SYS_PATH "/sys/class/plcm_usb/plcm0/f_audio_dual/pcm"
...
static unsigned int g_samplerate = (unsigned int)-1;
...
static int read_value_from_file(const char* filename, const char* format, ...)
{
    int fd = 0;
    char buf[8];
    va_list argptr;
...
int main(int argc, char* argv[])
{
    ...
    snd_pcm_uframes_t max_frame_num_of_period  = 0;
...
#define SSM_NAME "ssm_test"
    ...
    rc = read_value_from_file(UAC_AUDIO_DEVICE_SYS_PATH, "%d %d\n", &uac_card, &uac_device);

    if (rc < 0) {
        fprintf(stderr,	"[audio_play] unable to open uac pcm config: %s, error:%d\n",
                UAC_AUDIO_DEVICE_SYS_PATH, rc);
        snd_pcm_drop(handle);
        snd_pcm_close(handle);
        exit(1);
    }

    snprintf(uac_playback_pcm_name, sizeof(uac_playback_pcm_name), "hw:%d,%d", uac_card, uac_device);

    rc = snd_pcm_open(&phandle, uac_playback_pcm_name, SND_PCM_STREAM_PLAYBACK, 0);
...
#endif
        printf("[audio_capture] SND_PCM_FORMAT_S24_LE !\n") ;
...
#if 1
    dir = 0;
    snd_pcm_hw_params_get_period_size_max(params, &max_frame_num_of_period, &dir);
    printf("[audio_capture]max frames per period= %u (frames)\n", (unsigned int)max_frame_num_of_period);
#endif
    {
        ...
        snd_pcm_uframes_t tmp_frame_num_of_period;

        if (ratio == 0) ratio = 1;

        tmp_frame_num_of_period = hoped_frame_num_of_period * ratio;

        if (max_frame_num_of_period < tmp_frame_num_of_period)
            frame_num_of_period = 1024;
        else
            frame_num_of_period = tmp_frame_num_of_period;
    }
...
#define TWO_PI (3.1415926 * 2)
                ...
                if (p_mode > 0) {
                    ...
                    for (i = 0, j = 0; i < read_buffer_size && j < size; i += frame_size, j += sample_size) {
                        // Assume the sample_size is 16.
                        // memcpy((unsigned short *)wbuffers + j,
                        // (unsigned short *)(buffers[0] + i) + g_offset, sample_size);
                        for (t = 0; t < sample_size; t++)
                            wbuffers[j + t] = buffers[0][i + g_offset * sample_size + t];
                    }
                }
                ...
                printf("[audio_capture]wbuffers = %d wsize =%d frame_size %d read_buffer_size %d chn %d %lu s %lu ms\n", j, size, frame_size, read_buffer_size, g_offset, tpsend.tv_sec - tps.tv_sec, (tpsend.tv_usec - tps.tv_usec) / 1000000);
                clock_gettime(CLOCK_REALTIME, &tps);
...
int extract_channel_pcm(unsigned int channel_num, unsigned int period_size, unsigned int period_count, unsigned int sample_size, unsigned char *pcm_buffer)
{
    ...
    for (i = 0; i < frame_count; i++) {
        ...
        for (j = 0; j <  channel_num; j++) {
            for (t = 0; t < sample_size; t++) {
                channel_pcm_buffers[j][offset + t] = buffer[k + t];
            }
