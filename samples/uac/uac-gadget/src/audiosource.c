...
void audio_snd_card_write_dummy()
{
    usleep(20 * 1000);
...
static int audio_snd_card_trans(snd_pcm_t *pcm_handle, char* buffers)
{
    int rc = 0;
...
int audio_snd_card_write(snd_pcm_t *pcm_handle, char* buffers, int size)
{
    ...
    if (p_mode > 0) {
        audio_snd_buffer_fill(wbuffers, read_buffer_size);
    ...
    audio_snd_buffer_endian(wbuffers, buffers, read_buffer_size, FALSE);
#endif
    audio_snd_card_trans(phandle, wbuffers);
