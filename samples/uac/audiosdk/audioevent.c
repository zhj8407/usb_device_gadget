...
void start_audio_in(pid_t child)
{
    //if (child > 0)
...
int audio_snd_event(void *arg)
{
    ...
    int fd;
    struct linxed_struct* event = (struct linxed_struct *)arg;
    pid_t child = event->pid;
    sigset_t sigset;
...
int	audio_snd_event_get(struct linxed_struct* st)
{
    int status;
    pthread_t tr;
    printf("st->state: %d\n", st->state);
    pthread_create(&tr, NULL, audio_snd_event, (void *)st);
