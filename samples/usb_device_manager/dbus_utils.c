...
void watchHandle(DBusConnection *conn)
{
...
void handleListenFds(fd_set *rfds, fd_set *wfds, fd_set *efds)
{
    struct watchlist_t *l;
    int fd;
...
int setup_dbus_connection(DBusConnection **conn,
                          ...
                          const char *msg_match)
{
    ...
    *conn = dbus_bus_get_private(DBUS_BUS_SYSTEM, &err);
    ...
    if (!setupWatchFunctions(*conn)) {
        fprintf(stderr, "Failed to setup the watch functions\n");
        ret = -ENOMEM;
        goto out;
    }

    if (!setupTimeoutFunctions(*conn)) {
